...
넥슨의 게임 서버 구현은 클라 요청에 핸들링하고 공유 자원 접근할 때 동시성 문제 주의하고, 시간 의존적이면 요청 처리 측에서 자체 타이머를 세팅하고 등등... 이런 패턴이 왕도였던 모양이다. MM 서버에서 각 채널 업데이트 틱을 도입해서 병렬 스레드들이 채널을 하나씩 맡아서 update 를 돌리는 이런 구현은 낯설었는지, 넥슨 코리아 인프라팀에서 질문을 무지하게 많이 했다고 하더라. 맥락을 이해 못한 질문도 끼어 있어서 질문 하는 쪽도 답하는 쪽도 둘다 황당해 했다더라.