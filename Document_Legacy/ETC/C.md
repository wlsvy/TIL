...
- 원리가 뭐냐면 -> 그냥 무식하게 매 틱마다 `Application.CaptureScreenshot`(UnityAPI) 로 스크린샷을 찍어낸다. 압축 과정이 없기 때문에 생성되는 이미지 용량이 어마무시할 것.
...
**AWS Network NLB(Network LoadBalancer)**

[What is a Network Load Balancer - Elastic Load Balancing](https://docs.aws.amazon.com/elasticloadbalancing/latest/network/introduction.html#network-load-balancer-overview)
...
- 가용성
  - 재해 복구(Disaster Recovery, DR), 랜섬웨어(Ransomeware) 예방.
  - 인프라 단계에서 해결 본다, 가용영역 확장, 백신 프로그램 등.
- 무결성
  - 통신상 문제 혹 해킹등의 방해에 의한 변조 방지.
  - Checksum(네트워크 계층, 전송 계층), Hash Function( Hash를 통한 데이터 변조 체크)
- 기밀성
  - 평문이 아닌 암호문으로 통신. 암호화 기술
  - 양방향 암호화(암/복호화 과정을 통해 메시지를 안전하게 송수신)알고리즘을 사용.
  - 대칭키, 비대칭키

패킷 별 암호화 수준 결정.

- 사용 안함 : 암호화를 할 수 없거나, 필요 없는 경우 (최대한의 성능을 발휘해서 브로드캐스트 하는 경우)
- XOR : 암호화를 안 할 순 없지만 빠른 연산이 필요한 경우.
- AES-128 : 안정성 도모가 좀 더 필요한 경우
- HMAC – 1 : HMAC의 활용한 패킷 무결성 체크( without 암호화)
  - 비대칭-키 암호화 Key(public) 전달
- HMAC – 2 : HMAC의 활용한 패킷 무결성 체크( with 암호화)
  - 로그인(인증)? 등

## 23.10.17

**라이브 게임 서버가 다운될 수 있는 예상 원인들**

- 코드상의 메모리 누수
- 데드락
- 통신 패킷 사이즈가 너무 클 때
  - (실제 사례) 수천 개의 아이템이 있는 유저의 DB 데이터 로드
- 통신 어뷰징
...
- 이런 문제 특징 상 QA 에서 잡아내기가 굉장히 어렵다. 간헐적으로 발생하는 문제는 qa 테스터 문제 정의를 명확히 하기가 어려워서 그냥 넘기는 경우가 많기 때문
- 라이브 까지 갔으면?? 그래도 심각하게 다뤄지지는 않았을 듯. 그냥 가끔 터지는 미스테리한 문제 정도로 남게될까

## 23.10.31

[Span T를 사용해야 할 5가지 이유 - jacking75](https://jacking75.github.io/NET_Span_5_Reasons_to_Use/)
...
그러던 Span이 구현 되었다. 앞서 언급했듯이, Span은 3종의 배열 구조에서도 만들 수 있을뿐만 아니라 같게 다룬다. 인터페이스 타입과는 다르지만, "배열 타입"으로 기대되는 행동을 멋지게 추상화 되어 있다고 할 수 있겠다.

IList/ IReadOnlyList/ ICollection/ IEnumerable로 충분?

만약 그렇게 생각한다면, 대개의 경우 그 감각이 옳은 것 같다.
이미 언급 한 바와 같이, Span는 저수준 프로그래밍을 위해 존재한다. 대규모 응용 프로그램도 적당히 쉽게 보수성 좋게 쓸 수 있는 C#에서 원시 바이너리에 주목하는 케이스는 결코 많지 않다. 실체는 List가 되는것을 사용으로 두어서 적절한 인터페이스를 공개하도록 하는 것이 .Net 타입 시스템에서 솔직하고, 사용 측에 매우 편리하고 직관적인 것이 사상의 대부분이다. 성능이 매우 중요하다든가, 레거시 프로토콜을 구현하고 싶다든가, 명확한 이유가 없다면 Span을 쓸 필요는 없다.

마지막
Span은 어쨌든 사용하는 수준의 것이 아니지만, 속도가 중요한 프로그래밍에 확실히 힘을 실어 준다. 만약 당신이 I/O나 통신의 핵심 라이브러리를 쓰는 일에 관련하고 있다면, 꼭 사용하기 바란다.

**게임 데이터 ID 발급**

- 유일성을 보장해야 한다.
- 게임 라이브 기간 동안 데이터ID는 특별한 이유 없이 변경되선 안된다.
- 개발 기간 중 데이터 스키마 구조의 변경 / 확장 시 DB 마이그레이션이 필요할 수 있다.
- 휴먼 에러를 어느 정도 막을 수 있어야 한다.
  - 중복된 ID 입력
  - ID 한 칸 씩 밀려쓰기

**데이터 ID 수동 입력 방식**

- 기획자 등 비프로그래머는 데이터를 추가하고 개발명 문자열을 입력
- 문자열을 해쉬 함수로 정수열 ID 변환 <- 자동화 + 멱등성
- 충돌이 발생할 수 있지만 가능성이 희박하고, 그때는 개발명 문자열을 살짝 고쳐주면 될 것

**데이터 ID 자동 발급 방식**

- 기획자 등 비프로그래머는 데이터를 추가하고 개발명 문자열을 입력
- 데이터가 추가되면, 대응되는 Id 를 자동 생성
- 비프로그래머가 입력하는 개발명과 시스템이 발급하는 ID 사이에는 특별한 관계가 없음
- 데이터를 밀려쓴다던가 하면 시스템이 데이터 Id를 처음부터 다시 발급하는 일이 있음.
  - 이 문제 때문에 자동 -> 수동 입력 방식으로 회귀

## 23.11.08

**프레임 Update 중 게임 Component 갱신 순서 관련 오류**

컴포넌트 업데이트 중 컴포넌트는 다른 컴포넌트의 속성 값들은 계속 갱신할 수 있는 상태이다.

A -> B -> C  순으로 세가지 컴포넌트가 작동할 때 C가 동작하는 중에 A의 값을 수정할 수 있는 셈.

이 형태가 어떻게 문제가 되냐면, 위의 업데이트 순서가

B -> C -> A 로 바뀔 때 무슨 사이드 이펙트가 발생할지 사전에 예측할 수 없다는 것이다.
...
- '그 손모양' 형태를 연상시키기 때문에 수정 요청이 들어왔다.

## 23.12.20

닷넷 GC 호출 중에 데드락이 발생한 것으로 보이는 이슈가 있었는데, 

GC 중에 모든 스레드가 멈춰버리니 외부에서 보기에 로그도 남지 않고 오류도 없는 기현상이 발생

- .net 6.0 버그인 것 같다고 한다...
- 그래서 대응할 수 있는 방법이 뭐였나면... 닷넷 버전 올려서 다시 테스트하기
...
**대규모 인원이 작업하는 프로젝트에서 코드가 망가지는 사례 하나**

- 특정 영역에서 많은 기능이 만들어진다.
- 그 중 겹치는 기능에 대해 중복 코드가 자라난다. (사실 피할 수 없는 문제)
- 중간에 들어온 작업자는 특정 기능을 하는 코드를 고치다가, 동일한 행동을 하는 중복 코드의 존재를 모른 채 수정을 누락하게 된다.
- ( X _ X )
...
- Equals(object) / Equals(T)
  - 보통 위처럼 구현된다. 그래서 보통은 `Interger.Equals(OhterValue)` 에서 OtherValue와 Interger는 서로 다른 타입이어도 잘 컴파일 되고 동작함
...
[React & Javascript Optimization Techniques  by Rafael Rojas  Globant  Jan, 2024  Medium](https://medium.com/globant/javascript-optimization-techniques-20d8d167dadd)

1. Debounce (디바운스):

Debounce는 연속적으로 발생하는 이벤트 중에서 마지막 이벤트가 발생한 후에만 이벤트 핸들러를 호출합니다.
연이어서 발생하는 이벤트들 중에는 첫 번째 이벤트를 제외한 나머지 이벤트들을 무시하고, 일정 시간 동안 새로운 이벤트가 발생하지 않았을 때에만 이벤트 핸들러를 호출합니다.
사용자의 입력에 대한 반응을 제어하거나, 검색 기능에서 실시간으로 결과를 표시할 때 유용합니다.

2. Throttle (쓰로틀):

Throttle은 일정 주기 동안에 한 번만 이벤트 핸들러를 호출합니다.
일정 시간 동안 연속적으로 발생하는 이벤트 중에 첫 번째 이벤트만 처리하고, 그 후 일정 시간이 경과할 때까지 다음 이벤트를 무시합니다.
주로 스크롤 이벤트나 리사이징 같은 빈번한 이벤트의 처리를 제어하여 성능을 향상시킬 때 사용됩니다.
