...
- 간혹 동일한 구성으로 여러 개의 Task 객체를 생성하고 싶을 경우가 있습니다. Task생성자에 매번 동일한 매개변수를 전달하는 과정을 반복하여 Task 객체를 생성하는 것도 방법이겠으나, 이처럼 동일한 구성이 필요한 경우라면 태스크 팩토리(Task Factory)를 사용할 수 있습니다.
...
- TaskScheduler 객체는 태스크의 수행을 스케쥴 할 뿐 아니라 Visual Studio 디버거에 개별 태스크의 정보를 전달하는 역할도 수행합니다.
...
- 내부적으로 모든 Timer 객체에 대하여 단 하나의 스레드만이 할당되는데, 이 스레드는 다음 번 호출해야 할 Timer 객체의 수행 시점을 미리 알고 있어서 해당 시점이 되면 깨어나서 내부적으로 ThreadPool의 QueueUserWorkItme 메서드를 호출하여 스레드 풀의 큐에 항목을 삽입한다.
...
- 스레드 개수가 충분하지 않을 경우, 기아 현상이나 데드락이 발생할 수 있기 때문에 CLR팀은 스레드 풀이 가질 수 있는 최대 스레드 개수를 증가시켜 왔습니다.
...
- 웹 응용 프로그램 같은 경우, I/O 작업을 수행할 때 비동기적으로 진행한다. 그 이유는 첫째로 응답성의 측면에서 동기적 처리는 좋지 않을 수 있기 때문. 두 번째 이유는 I/O 작업을 동기적으로 수행할 시 커널 모드에 진입하는 순간 사용자 스레드가 전부 정지되었다가 I/O 작업이 끝나면 깨어나게 되는데, 이때 블로킹 되어 있던 스레드가 모두 깨어나서 컨텍스트 전환 비용기 급격하게 늘어나게 된다.
- 리소스 사용량과 컨텍스트 전환의 횟수를 최소화하는 것에 더하여, I/O 작업을 비동기적으로 수행하면 다른 장점들도 있는데
  - 가비지 수집하는 과정에서 살펴봐야할 스레드의 개수가 줄어든다.
  - 디버깅 과정 중 브레이크 포인트 진입시, 모든 스레드가 정지하는 경우 -> 여기서도 오버헤드가 줄어든다.
  - 다수의 작업을 동기적으로 수행하면 개별 작업의 수행시간을 전부 합한만큼 걸리지만, 비동기로 수행되는 경우 개별 작업 중 가장 오랜 시간이 소요되는 작업을 수행하는 시간 정도면 모든 작업을 완료한다.
