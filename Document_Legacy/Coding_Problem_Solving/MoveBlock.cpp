...
/*
    로봇의 '특정 지점을 방문할 때까지 걸린시간?'을 체크할때 고생한 케이스입니다.
    
    회전 상태에 따라 다르게 확인해 줘야 할 필요가 있었는데 
    처음에 회전 상태를 고려하지 않고 코드를 작성한 것이 2시간!! 씩이나 시간을 낭비했습니다.
    
    (로직이 복잡한 케이스라 오류가 생기면 원인을 찾는데도 시간이 꽤나 걸리니....)
    
    결론은 역시 배열. 
    int Dist[2][200][200]; (회전 상태 - 2, 최대 x 범위 - 200, 최대 y 범위 - 200) 이런식으로
    회전상태에 따라서 구분한게 역시 가독성 면에서는 도움이 되더군요.
*/
