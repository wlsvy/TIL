...
* **언제 쓰나요?** 파일이 너무 많아 `git status`가 눈에 띄게 느릴 때 사용합니다.
* **추천 설정:** 우선 `git update-index --test-untracked-cache`로 시스템 호환성을 확인한 후, `git config core.untrackedCache true`를 적용해 보세요.

Git은 `git status`를 실행할 때 작업 디렉토리를 스캔하여 "추적되지 않는 파일(untracked files)"이 있는지 확인합니다. 파일 수가 수십만 개 이상인 경우, 운영체제에 모든 디렉토리의 파일 목록을 요청하는 과정에서 상당한 시간이 소요됩니다.

Untracked Cache는 이 **디렉토리 스캔 결과**를 인덱스(`index`) 파일에 캐싱합니다. 다음번 검사 때 디렉토리의 수정 시간(**mtime**)이 변하지 않았다면, 내부를 다시 뒤지지 않고 캐시된 결과를 그대로 사용하여 속도를 높입니다.

| 설정값              | 동작 방식                                                                   |
| ---                 | ---                                                                         |
| **`keep`** (기본값) | 기존에 캐시가 활성화되어 있다면 계속 유지하고, 없으면 새로 만들지 않습니다. |
| **`true`**          | 기능을 활성화합니다. 인덱스에 캐시 데이터가 자동으로 추가됩니다.            |
| **`false`**         | 기능을 비활성화합니다. 인덱스에서 관련 캐시 데이터를 자동으로 제거합니다.   |

* **참고:** `feature.manyFiles` 설정을 켜면 이 값이 자동으로 `true`가 됩니다.

**활성화 전 필수 체크: `mtime` 확인**

이 기능은 **"디렉토리의 mtime이 변하면 그 안의 파일도 변했다"**는 가정하에 작동합니다. 따라서 운영체제와 파일 시스템이 디렉토리 수정 시간을 정확하게 업데이트해야만 신뢰할 수 있습니다.

Git은 이를 테스트할 수 있는 명령어를 제공합니다. `true`로 설정하기 전에 반드시 실행해 보세요.
...
결과에 **"OK"**가 나오면 사용해도 안전합니다. 만약 문제가 있다면 캐시 때문에 실제로는 파일이 추가되었는데 `git status`에 나타나지 않는 오류가 발생할 수 있습니다.

**4. 수동 제어 방법**

설정 파일(`.gitconfig`)을 고치지 않고 인덱스에 직접 명령을 내려 캐시를 생성하거나 제거할 수도 있습니다.

* **캐시 생성:** `git update-index --untracked-cache`
* **캐시 제거:** `git update-index --no-untracked-cache`
...
* 보통 Git은 변경 사항을 찾기 위해 작업 디렉토리의 모든 파일을 뒤져봅니다.
* `fsmonitor`를 켜면 OS의 파일 시스템 모니터링 서비스(Windows의 경우 `fsmonitor--daemon`)로부터 **"마지막 확인 이후 변경된 파일 목록"**을 직접 전달받습니다.
* 따라서 Git은 변하지 않은 수십만 개의 파일을 무시하고, 변경된 파일만 즉시 확인합니다.

### fscache (file system)

Git에서 **fscache**는 주로 Windows용 Git(Git for Windows) 환경에서 파일 시스템의 응답 속도를 높이기 위해 도입된 성능 최적화 기능입니다.

리눅스와 달리 윈도우의 파일 시스템 API는 파일 상태를 조회(lstat, fstat 등)할 때 오버헤드가 상대적으로 큽니다. fscache는 이 문제를 해결하기 위해 고안되었습니다.
...
## Git object의 분류와 저장 방식

아래 내용은 Git의 물리적/논리적 저장 구조와 핵심 최적화 명령어의 내부 작동 방식을 설명한 사전 지식입니다. 생소한 용어가 나올 때마다 참고하시면 구조를 이해하는 데 도움이 됩니다.

Git은 프로젝트의 모든 데이터를 'Git object'라는 단위로 분류하여 저장합니다.

#### 논리적 단위
* **Blob (Binary Large Object)**: 파일의 메타데이터를 제외한 순수 바이너리 데이터만을 압축해서 담고 있습니다.
* **Tree**: 파일 시스템의 폴더 역할을 담당합니다. 파일명, 메타데이터, 해당 폴더 내 파일들의 blob 해시값, 그리고 하위 폴더 tree의 해시값 등을 담고 있습니다.
* **Commit**: 프로젝트의 특정 시점에 대한 스냅샷이며 해당 스냅샷에 대한 메타데이터를 포함합니다. 커밋한 사람, 시간, 커밋 메시지 정보와 최상위 tree 오브젝트 해시값, 그리고 이전 커밋(부모 커밋)의 해시값을 가지고 있습니다.
* **Tag**: 특정 커밋을 가리키는 포인터 역할을 하며 태그 이름, 작성자, 메시지, 대상 커밋의 해시값 등을 담고 있습니다.

#### 실제 저장 형태
위의 논리적 오브젝트들은 디스크 상에 아래 두 가지 방식 중 하나로 물리적 저장 공간을 점유합니다.

* **Loose object**: **파일 하나당 오브젝트 하나를 매핑하여 개별 파일로 저장**하는 방식입니다. 오브젝트 해시값의 앞 두 글자를 폴더 이름으로 삼고, 나머지 글자를 파일 이름으로 삼아 분산 저장합니다.
  * 새로운 오브젝트를 만드는 속도가 매우 빠르다는 장점이 있습니다. 이전 버전 파일과의 연관성을 검사하지 않고 바로 생성하기 때문입니다.
  * 로컬에서 작업한 내용을 커밋하면 기본적으로 loose object로 생성됩니다. 이후 별도의 최적화 명령을 수행하기 전까지는 계속 개별 파일 형태로 디스크에 남아 있게 됩니다.
  * **프로젝트가 커질수록 개별 파일 수가 기하급수적으로 늘어나** 파일 입출력 성능을 떨어뜨리고 디스크 공간을 낭비하는 원인이 됩니다.
* **Packed object**: **수많은 개별 오브젝트를 하나의 묶음 파일**(`.pack`)로 압축해서 저장하는 방식입니다.
  * 여러 파일이 묶여 있으므로 특정 오브젝트를 신속하게 찾을 수 있도록 돕는 **인덱스 파일**(`.idx`)이 1:1로 함께 생성됩니다.
  * 동일한 파일의 이전 버전과 현재 버전 간의 차이점만 기록하는 델타 압축 기술을 적극적으로 활용하여 공간을 크게 절약합니다.

### 2. Promisor pack의 역할

Partial clone 환경에서 로컬 저장소의 무결성을 유지하기 위해 사용하는 핵심 기능입니다.

* Partial clone을 사용하면 대용량 파일의 알맹이(blob)가 빠진 채로 메타데이터만 다운로드받게 됩니다. 본래 일반적인 Git 환경에서는 연결된 오브젝트가 빠져 있으면 저장소 손상 에러를 반환해야 합니다.
* 이러한 에러를 방지하고 **누락된 파일이 원격 저장소에 존재한다는 것을 보증하기 위해 생성하는 마킹 파일**이 바로 `.promisor` 파일입니다. 이 파일이 존재하면 **Git은 특정 오브젝트가 로컬에 당장 없더라도 "필요할 때 언제든 원격에서 안전하게 받아올 것"이라고 인식**하여 오류를 내지 않습니다.
* 저장소 내부에는 promisor pack 외에도 임시 격리용 cruft pack, 수동 보호용 keep pack 등 다양한 성격의 팩 파일이 공존합니다.

### 3. Midx (Multi-Pack-Index)의 도입 이유

Packed object가 늘어나면 원하는 파일을 찾기 위해 수많은 개별 `.idx` 파일을 전부 읽어야 하므로 탐색 성능이 저하됩니다.

* 이 문제를 근본적으로 해결하기 위해 모든 개별 인덱스를 다시 풀어서 하나의 팩으로 재압축하는 작업(Repack)은 막대한 물리적 시간과 연산 비용이 소요됩니다.
* 이에 대한 대안으로 제시된 기술이 바로 **midx(multi-pack-index)**입니다. 기존의 `.pack`과 `.idx` 파일들을 그대로 유지한 상태에서, 모든 개별 인덱스 내용을 단 하나의 파일 안에 취합하여 정리해 둡니다.
* midx가 구축되면 Git은 수백 개의 개별 인덱스 파일을 뒤질 필요 없이 단 하나의 midx 파일만 순회하여 목적 파일을 빠르게 찾게 됩니다.
* 만약 여러 팩 파일 내에 중복된 오브젝트가 존재할 경우, midx는 아래와 같은 우선순위를 적용하여 대상 오브젝트를 지정합니다.
  1. `--preferred-pack` 옵션으로 지정된 기본 팩을 최우선으로 선택합니다.
  2. 최종 수정 시간(mtime)이 가장 최신인 팩 파일을 선택합니다.
  3. 팩 파일의 ID 값이 낮은 것(작은 숫자)을 선택합니다.

### Partial Clone 을 사용하는 프로젝트에서 발견됐던 문제점

partial clone 방식을 사용하는 프로젝트 - 10KB를 넘는 대용량 파일(blob)은 클론 시점에 로컬에 받지 않고, 실제 필요할 때 원격 저장소에서 동적으로 가져오는 lazy-fetch 방식을 활용.

이 방식의 특징과 문제점은 다음과 같습니다.

* **동적 팩 생성과 누적**: 파일 상태를 확인하거나 다운로드할 때마다 **원격 저장소에서 새로운 `.pack` 파일(promisor pack)을 생성하여 로컬로 보냅니다**. **브랜치를 전환하거나 fetch, diff 등을 실행할 때마다 새로운 팩이 계속 생깁니다**.
* **자동 정리 비활성화**: 프로젝트 정책으로 자동 가비지 컬렉션(`gc.auto=0`)을 강제하므로, 늘어난 팩들이 자동으로 합쳐지지 않고 계속 쌓입니다. 
* **파편화의 비효율**: 
  1. **용량 증가**: 동일한 에셋의 여러 버전이 여러 `.pack` 파일에 조각나서 저장되면 **파일 간 차이를 계산하는 델타 압축 효율이 떨어집니다.** 결과적으로 전체 팩 파일의 크기가 불필요하게 커집니다.
  2. **성능 저하**: 파일 위치 정보를 담은 **인덱스(`.idx`) 파일 개수도 팩 파일 개수와 동일하게 늘어납니다.** 특정 파일을 찾기 위해 순회해야 하는 인덱스가 많아져 전반적인 파일 탐색과 git status, checkout 속도가 떨어집니다.
...
| 작업 | 목적 | 주요 기능 | 주의사항 |
|------|------|-----------|----------|
| **commit-graph** | 커밋 그래프 최적화 | • 커밋 그래프 파일을 점진적으로 업데이트 • 동시 실행 중인 Git 프로세스와 안전하게 병행 가능 | 기존 `.graph` 파일은 지연 삭제됨 |
| **prefetch** | 원격 객체 미리 가져오기 | • 모든 등록된 원격 저장소에서 객체 미리 다운로드 • `refs/prefetch/` 경로에 저장하여 기존 브랜치 보호 • 실제 fetch 시 속도 향상 | `remote.<name>.skipFetchAll`로 특정 원격 제외 가능 |
| **gc** | 가비지 컬렉션 | • 불필요한 파일 정리 및 저장소 최적화 • 모든 Git 객체를 단일 팩 파일로 재포장 | **대용량 저장소에서 비용이 많이 듦** 오래된 데이터 삭제로 인한 중단 가능성 |
| **loose-objects** | 느슨한 객체 정리 | • 느슨한 객체를 팩 파일로 이동 • 2단계 프로세스로 경쟁 상태 방지 • 배치 크기 제한 (5만 개 객체) | **gc 작업과 동시 활성화 비권장** |
| **incremental-repack** | 점진적 재포장 | • multi-pack-index 기능으로 객체 디렉토리 재포장 • 작은 팩 파일들을 큰 팩 파일로 병합 • 2단계 프로세스 (expire → repack) | 배치 크기 설정 가능 (기본값 0은 전체 병합) |
| **pack-refs** | 참조 파일 압축 | • 느슨한 참조 파일들을 단일 파일로 수집 • 다수 참조 순회 작업 속도 향상 | - |
...
- [Git - git-ls-tree Documentation](https://git-scm.com/docs/git-ls-tree)

- Git은 **콘텐츠 주소 지정 가능 파일 시스템(Content-addressable filesystem)**입니다. 즉, 이름이 아니라 **내용물**을 가지고 ID를 만듭니다.

### **Blob ID (파일)**

파일의 이름이나 수정 시간은 무시하고 **오직 파일의 내용**만 가지고 만듭니다.

* **공식:** `sha1("blob " + 파일크기 + "\0" + 파일내용)`
* **특징:** 파일명이 달라도 내용이 100% 같으면 동일한 Blob ID를 가집니다.

> **참고:** Git은 보안 강화를 위해 최신 버전에서 SHA-256으로의 전환을 준비하고 있습니다.

### **Tree ID (디렉토리)**

디렉토리 내의 **구성 요소 목록**을 가지고 만듭니다.

* **포함 데이터:** 하위 파일/디렉토리의 `모드(권한)`, `타입(blob/tree)`, `해시값`, `이름`
* **공식:** 위 목록을 정렬하여 하나의 텍스트로 만든 후 SHA-1 해시를 수행합니다.
* **특징:** `diff`가 동일하다는 것은 파일 내용, 이름, 구조가 모두 같다는 뜻이므로 **Tree ID도 반드시 동일**하게 나옵니다.

* **diff가 없음 (동일) -> Tree ID 동일 (100%)** : 해시 함수는 결정론적(Deterministic)이므로 입력(내용/구조)이 같으면 결과도 같습니다.
* **diff가 있음 (다름) -> Tree ID 다름 (~100%)** : SHA-1의 눈사태 효과(Avalanche effect)로 인해 단 1비트만 달라도 해시값은 완전히 달라집니다.
