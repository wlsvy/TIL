...
Git이 파일(Blob)과 디렉토리(Tree)의 해시(ID)를 생성하는 원리와 충돌 확률에 대해 명확히 정리해 드립니다.

---

### 1. ID 생성 방법 (Hashing Process)
...
* **지구상의 모래알:** 지구상의 모든 모래알 개수는 약 $10^{18}$개로 추정됩니다. Git의 해시 범위는 이보다 비교할 수 없이 큽니다.
* **확률 수치:** 매초 650만 개의 객체를 생성한다고 가정할 때, 우주가 멸망할 때까지 생성해도 충돌이 일어날 확률은 로또 1등에 연속으로 수십 번 당첨될 확률보다 낮습니다.

#### **왜 "거의"인가? (이론적 한계)**

* **비둘기집 원리:** 입력값(파일 내용)은 무한하지만, 결과값(해시)은 유한하므로 이론적으로는 다른 내용인데 같은 해시가 나올 수 있습니다.
* **SHAttered 공격:** 2017년 구글 연구진이 의도적으로 조작된 두 PDF 파일의 SHA-1 해시를 같게 만드는 데 성공했습니다. 하지만 이는 수천 대의 CPU/GPU를 동원한 특수 사례이며, 일반적인 개발 과정에서 우연히 발생할 확률은 여전히 **0**에 수렴합니다.

---

### 3. 요약: Diff와 Tree ID의 관계
...
혹시 특정 커밋들 사이의 Tree ID 차이를 직접 확인하는 명령어나, 충돌 방지를 위한 Git의 내부 검증 로직에 대해 더 알고 싶으신가요?
